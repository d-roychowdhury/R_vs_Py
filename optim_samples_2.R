...
packages <- c("expm", "fastmatrix", "matrixNormal", "fastmatrix")
...
##------------------------------------------- the function defined based on pre calculation of g1, gwt, nw-----------------------------------------------
g1_gwt_mat <- lapply(
                     X = 1:NB,
...
ith_boot_function_2<- function(i){ ## based on previous computation of g1, gwt, nw
...
A_min <- function(i){ 
  ...
  neg_lpda <- ith_boot_function_2(i)
...
objects_to_export <- c("Psi", "Psi0", "K_inv", "L_inv", "M", "A0", "A_true", "nu", "nu0", "n", "p", "r", "u", "A_test", "A_env", "init_pts", "NB",
                       "Nw_vec", "g1_mat", "gwt_mat", "ith_boot_function_2", "A_min")
...
f_vals <- sapply(X=c(1:NB), FUN = function(x){
                                  neg_lpda <- ith_boot_function_2(x)
