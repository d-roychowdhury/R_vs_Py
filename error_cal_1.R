...
#A_opt <- lst_A_opt_futlap_final  # removed on April 13th
...
comparisons_wbb <- function(NB,A_arg){ # where t is the time taken to calculate A_arg
  ...
  err_bayes_beta2 <- mean(sapply(X=c(1:NB), FUN= function(x) return(norm(beta_sample2[[x]]-beta_true, type ="2")) ))
   err_freq_beta_env <- norm(beta_env-beta_true, type="2") 
   #normvec(beta_env - beta_true)
  ...
  return(list("NB" = NB,
              ...
              "err_freq_beta_cbb" = err_freq_beta2,
